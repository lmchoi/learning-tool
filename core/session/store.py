...
class SessionStore:
    ...
    def record(self, session_id: str, question_text: str, score: int) -> None:
    ...
    def add_attempt(self, attempt: QuestionAttempt) -> None:
        with sqlite3.connect(self._db_path) as conn:
            conn.execute(
                "INSERT INTO attempts (session_id, question_text, score, timestamp)"
                " VALUES (?, ?, ?, ?)",
                (attempt.session_id, attempt.question_text, attempt.score, attempt.timestamp),
            )
        ...
        with sqlite3.connect(self._db_path) as conn:
            ...
            for session_id, context, started_at in rows:
                attempt_rows = conn.execute(
                    "SELECT session_id, question_text, score, timestamp"
